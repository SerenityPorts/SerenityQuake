...
/*
=====================
D_SpriteDrawSpans_Translucent
=====================
*/
void
D_SpriteDrawSpans_Translucent(sspan_t *pspan)
{
    int count, spancount, izistep;
    int izi;
    byte *pbase, *pdest;
...
// we count on FP exceptions being turned off to avoid range problems
	...
	do {
		...
		if (btemp != 255 && *pz <= (izi >> 16)) {
                    *pdest = r_transtable[(((int)*pdest) << 8) + btemp];
		}
...
// wrapping
    ...
    D_SpriteScanRightEdge();
    if (r_transtable) {
        D_SpriteDrawSpans_Translucent(sprite_spans);
    } else {
